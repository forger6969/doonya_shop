...
interface Msg { id: string; from: string; text: string; ts: string }

interface Props {
  onClose: () => void;
}

export default function SupportChatSheet({ onClose }: Props) {
  const { t } = useLang();
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 flex flex-col gap-2 overscroll-contain">
          {messages.length === 0 && (
            <div className="flex flex-col items-center gap-3 py-16" style={{ color: "var(--text-muted)" }}>
              <Headphones className="w-10 h-10" style={{ opacity: 0.3 }} />
              <p className="text-sm text-center">{t.supportHint}</p>
            </div>
          )}
          {messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
                ...
                >
                  ...
                  <p className="break-words" style={{ color: isUser ? "#fff" : "var(--text)" }}>{msg.text}</p>
